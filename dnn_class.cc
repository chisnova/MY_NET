...
void DNN()
{
	...
	while(iteration<tr_iter){
		...
		//for(int k=0;k<60000/batch_size;++k){
		for(int k=0;k<500;++k){
			for(int i=0;i<batch_size;++i){
				idx=rand()%60000;
				for(int j=0;j<32;++j){
			...
			for(int i=0;i<10;++i) x[layer_size-1]->err[i]=0;
			for(int batch=0;batch<batch_size;++batch){
				...
				for(int i=0;i<10;++i){
					x[layer_size-1]->err[i]+=(x[layer_size-1]->val[i]-batch_label[batch][i])/(double)batch_size;
					mse+=(x[layer_size-1]->val[i]-batch_label[batch][i])*(x[layer_size-1]->val[i]-batch_label[batch][i]);
				}
			}
